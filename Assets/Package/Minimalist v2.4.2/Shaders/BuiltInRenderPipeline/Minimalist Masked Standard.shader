shader "Minimalist/MaskedStandard" {
	Properties{
		//Texture Module
	...
	SubShader{	
		...
		Pass{
			...
			fixed4 frag(vertexOutput i) : COLOR
			{
				...
				#if TEXTUREMODULE_ON
					fixed4 _MainTexture_var = tex2D(_MainTexture, i.uv) + fixed4(_MainTexturePower, _MainTexturePower, _MainTexturePower, 0);
				...
				//Calculating Shadows with shadow color
					...
					Result = fixed4(lerp(Result.r, maxVal, 1-_Saturation), lerp(Result.g, maxVal, 1-_Saturation), lerp(Result.b, maxVal, 1-_Saturation), Result.a);
				#endif
