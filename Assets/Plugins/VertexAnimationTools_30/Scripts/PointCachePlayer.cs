                ...
                for (int c = 0; c < pointCache.PostImport.UsedClipsCount; c++) {
                    ...
                    if (Application.isPlaying) {
                        ...
                        if (clip.AutoPlaybackType == AutoPlaybackTypeEnum.Repeat) {
                            float persentageDelta = dt / Mathf.Clamp(clip.DurationInSeconds, 0.001f, float.MaxValue);
                            clip.NormalizedTime += persentageDelta;
                            ...
                            float persentageDelta = dt / Mathf.Clamp(clip.DurationInSeconds, 0.001f, float.MaxValue);
                            clip.NormalizedTime += persentageDelta * timeDirection;
                            if (clip.NormalizedTime > 1f) {
        ...
        void OnDrawGizmos() {
            ...
            for (int c = 0; c < pointCache.PostImport.UsedClipsCount; c++) {
                ...
                for (int p = 0; p < pcclip.MotionPathsCount; p++) {
                    //first frame icon
                    Vector3 a = pcclip.GetGizmoPoint(p, 0);
                    Vector3 b = pcclip.GetGizmoPoint(p, 1);
                    Vector3 dir = b - a;
                    float magnitude = dir.magnitude;
                    dir = dir / magnitude;
                    dir *= iconSize*4;

                    Vector3 v0 = a + Vector3.Cross(dir, Vector3.up);
                    Vector3 v1 = a + Vector3.Cross(dir, Vector3.left);
                    Vector3 v2 = a + Vector3.Cross(dir, Vector3.down);
                    Vector3 v3 = a + Vector3.Cross(dir, Vector3.right);
